...
enum Api {
  ...
  deletePolymerizeCategory = '/polymerize/category/delete',
 ...
 * 删除
 */
export const deletePolymerizeCategory = (params,handleSuccess) => {
  return defHttp.delete({url: Api.deletePolymerizeCategory, params}, {joinParamsToUrl: true}).then(() => {
    handleSuccess();
  });
}
/**
 ...
 * @param params
 */
export const batchDeletePolymerizeCategory = (params, handleSuccess) => {
  createConfirm({
    iconType: 'warning',
    title: '确认删除',
    content: '是否删除选中数据',
    ...
    cancelText: '取消',
    onOk: () => {
      return defHttp.delete({url: Api.deletePolymerizeCategory, data: params}, {joinParamsToUrl: true}).then(() => {
        handleSuccess();
      });
    }
  });
